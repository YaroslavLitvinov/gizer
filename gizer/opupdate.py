...
def update (dbreq, schema_e, oplog_data, schema_name, database_name):
    ...
    if not updating_obj[-1].isdigit():
        ...
        if type(u_data[k]) is list:
            del_stmnt = delete(dbreq, schema, upd_path_str, doc_id.itervalues().next(), schema_name, database_name)
